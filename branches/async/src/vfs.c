...
vfs_enumerate_children_callback (GObject      *source_object,
    ...
    gboolean  show_hidden = ((Data*)data)->show_hidden;
    ...
    if (error) {
        ...
        utils_gerror_ok (&error, FALSE);
        data->callback (NULL, NULL, error_msg, data->data);
        g_free (data);
        return;
    }
    ...
    while ((file_info = g_file_enumerator_next_file (enumerator, NULL, &error)) != NULL) {
        ...
        vfs_file_info->file_name = g_strdup_printf ("%s/%s", data->parent_dir, g_file_info_get_name (file_info));
    ...
    data->callback (dirs, files, error_msg, data->data);
...
vfs_get_dir_listings_async (gchar *path,
    ...
    Data *_data = g_new0 (Data, 1);

    _data->data = data;
    _data->show_hidden = show_hidden;
    _data->callback = callback;
    _data->parent_dir = path;
    ...
    g_file_enumerate_children_async (file,
                                     ...
                                     G_PRIORITY_DEFAULT,
                                     cancellable,
                                     (GAsyncReadyCallback)vfs_enumerate_children_callback,
                                     (gpointer)_data);
